...
try {
  ...
  onAuthStateChanged(auth, user => {
    if (!user) { bounceToLogin(); return; }
    try { sessionStorage.removeItem("tbh_bounce"); } catch (e) {}
    reveal();
    paintUser(user);
    startTimer();
  });
...
/* ----------------------------------------------------- redirect-loop breaker
   If the browser will not keep the Firebase session (private window, blocked
   storage, third-party cookies off), login.html sends you here and this file
   sends you straight back — forever. After a few round trips, stop and say so
   instead of flickering between two pages. */
function bounceToLogin() {
  let n = 0;
  try {
    n = (parseInt(sessionStorage.getItem("tbh_bounce") || "0", 10) || 0) + 1;
    sessionStorage.setItem("tbh_bounce", String(n));
  } catch (e) { n = 1; }               /* storage blocked — treat as a single try */
  ...
  if (n > 3) {
    ...
    if (v) {
      ...
      v.innerHTML = '<div style="text-align:center;max-width:460px;line-height:1.65">' +
        '<b style="color:#1E1E1E">Signed in, but the session is not being kept.</b><br>' +
        'The browser is discarding the login, so this page keeps bouncing back to sign-in.<br><br>' +
        'Usually: a private / incognito window, blocked site data, or third-party cookies turned off.<br>' +
        '<a href="diagnose.html" style="color:#C8102E;font-weight:600">Run the diagnostic</a> &nbsp;·&nbsp; ' +
        '<a href="login.html" style="color:#C8102E;font-weight:600">Back to sign-in</a></div>';
    }
    return;
  }
